...
| `replay_live.py` | Replay mode — feed historical bars through the live engine at any speed. Two modes: legacy `--strategy <yaml>` (single asset) and `--config <multi-asset yaml>` (BTC + ETH together, funding filter, portfolio cap — Issue #26; vol_sizing parity — Issue #34). Matches live closed-bar semantics (Issue #24) and live fill model (Issue #29). Optional `--trades-out` CSV. Educational / intuition-building tool. Does not trade, does not write to `state/`. | #26, #34 |
