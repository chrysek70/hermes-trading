...
| `run_supertrend_long_short.py` | SuperTrend(10, 3) long-only vs long-short comparison. 6 variants (BTC / ETH / parallel × long-only / long-short). Single-asset variants reuse `walk_forward`; parallel coordinator written inline because the other parallel coordinators in this repo are long-only. | #19 |
...
| `run_adaptive_sizing.py` | Adaptive sizing layer on the adopted long-short + funding candidate. 4 variants: baseline, HMM sizing, vol-quartile sizing, stacked. Issue #27 mapping {1.0, 0.5, 0.25}. Per-fold per-asset HMM fit; per-fold vol-quartile thresholds (train-only). By-regime breakdown for HMM state and vol band. **All three sizing variants pass the adoption criterion; vol_sizing is the strongest candidate.** | #27 |
...
| `replay_live.py` | Replay mode — feed historical bars through the live engine at any speed. Two modes: legacy `--strategy <yaml>` (single asset) and `--config <multi-asset yaml>` (BTC + ETH together, funding filter, portfolio cap — Issue #26; vol_sizing parity — Issue #34; volume confirmation parity — Issue #38). Matches live closed-bar semantics (Issue #24) and live fill model (Issue #29). Optional `--trades-out` CSV. Educational / intuition-building tool. Does not trade, does not write to `state/`. | #26, #34, #38 |
